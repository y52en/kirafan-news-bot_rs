...
#[allow(non_upper_case_globals, dead_code)]
static re_js_img_link: Lazy<Regex> = Lazy::new(|| compiled_regex(r#"src="([^?"]+)"#));
#[allow(non_upper_case_globals, dead_code)]
...
pub async fn savenews(
    ...
    url: String,
    folder: String,
    process_list: &Arc<Mutex<Vec<tokio::task::JoinHandle<()>>>>,
) {
    let html = get_html_retry(&path, 5).await;
    let doc = parse_html(&html);

    let js_link = scraping(&re_js_img_link, &sel_js_link, &doc);
    let css_link = scraping(&re_css_link, &sel_css_link, &doc);

    let img_link = scraping(&re_js_img_link, &sel_img_link, &doc);
    ...
    for js in js_link {
        ...
        let mut process_list_locked = process_list.try_lock().unwrap();
